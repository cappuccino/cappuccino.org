...
* (b59b025) Fixed: `CPDateFormatter` did not work with the symobols `MM` and `LL` for the month of December.
...
* (448c2a2) New: speed up app loading by creating the `DOMCanvas` only if `CPView -drawRect:` or `CPView -viewWillDraw` are overriden, and do it lazily (on the first `viewWillLayout`).
...
* (027a317) Fixed: memory leaks with tooltTips in `CPView`. This was due to handlers not being released.
...
* (f2c5515) Fixed: `CPTableView -reloaData` caused an extra run loop perform.
...
* (e214bf3) Fixed: refaxtored `CPString -charPositionOfString:withFont:forPoint:`.
...
* (8908fb7) Fixed: view hiearchy was not updated while processing events and using the `CPTrackingArea`.
...
* (b432bd2) New: `Cucapp` now generates an error message for every `objj` exceptions catched.
...
* (3472386) New: possiblity to create screenshots from Cucapp.
...
The changes above summarise a few hundred of the most important changes in Cappuccino 0.9.9. You can review the other 250 or so changes in the [0.9.8...0.9.9 compare view](https://github.com/cappuccino/cappuccino/compare/0.9.8...0.9.9).
